...
function testEmail() {
  ...
  Logger.log('testEmail 完成');
}

// ── GET 請求處理（表單送出 + 後台讀取 共用）─────────────────
function doGet(e) {

  // ① 表單送出：?action=save&data={...}
  if (e.parameter && e.parameter.action === 'save') {
    try {
      const data = JSON.parse(e.parameter.data || '{}');
      Logger.log('doGet save：name=' + data.name + '  email=' + data.email);
      if (data.name) {
        saveRecord(data);
...
function saveRecord(data) {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1,1,1,headers.length)
         .setFontWeight('bold').setBackground('#0a2218').setFontColor('#ffffff');
  }
...
function sendResultEmail(data) {
  ...
  try {
    ...
    const PERS = {
      flower:{ type:'浪漫滋養型', emoji:'🌹', suggest:'建議每日以依蘭或真正薰衣草進行睡前香氛儀式，重建與自己的深層連結。' },
      citrus:{ type:'活力陽光型', emoji:'🍊', suggest:'工作前擴香甜橙或佛手柑，注入正向能量，清新柑橘氣息能快速轉換心情。' },
      herb:  { type:'清晰理性型', emoji:'🌿', suggest:'午後疲勞時塗抹胡椒薄荷或迷迭香於太陽穴，提振專注力，效果比咖啡更溫和持久。' },
      wood:  { type:'深根沉穩型', emoji:'🪵', suggest:'睡前以雪松進行深呼吸冥想，降低皮質醇、穩定自律神經，改善睡眠品質。' },
      fresh: { type:'自由清明型', emoji:'🌬️', suggest:'密閉空間工作時使用檸檬香茅擴香，提升清新感與專注力，減少悶塞感。' },
    };
...
.r2{display:flex;border-bottom:1px solid #eee}.cell{flex:1;text-align:center;padding:18px 12px;border-right:1px solid #eee}.cell:last-child{border-right:none}
.s{padding:0 28px 20px}.st{font-size:11px;font-weight:700;color:#1a5c3a;letter-spacing:.1em;text-transform:uppercase;margin:20px 0 8px}
